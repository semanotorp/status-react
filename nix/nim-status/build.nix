...
in stdenv.mkDerivation rec {
  ...
  inherit src;
  buildInputs = with pkgs; [ libtool wget git gcc which nim tcl];
  ...
  buildPhase = ''
    ...
    make nim_status_go V=3 SQCIPHER_CFLAGS="${sqliteCompilerFlags}" SQCIPHER_LDFLAGS="${sqliteLinkerFlags}" CFLAGS="${compilerFlags}" LDFLAGS="${linkerFlags}" CC=clang OS=${nimHostOs} USE_SYSTEM_NIM=1  PCRE_INCLUDE_DIR=${pcre}/include PCRE_LIB_DIR=${pcre}/lib SSL_INCLUDE_DIR=${openssl}/include SSL_LIB_DIR=${openssl}/lib 
   '';

  installPhase = ''
    mkdir -p $out
    cp vendor/nimbus-build-system/vendor/Nim-csources/c_code/nimbase.h $out/
    mv build/nim_status_go.a $out/libnim_status.a
    mv build/nim_status_go.h $out/nim_status.h
  '';
}
